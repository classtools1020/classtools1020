...
test('驗收 2：未授權者不能寫入；輸入人員不能越權發布或編輯未分配項目；複核人員不能編輯草稿', async () => {
  ...
  assert.equal((await anon.call('POST', `/api/admin/users`, { name: 'x', role: 'admin' })).status, 401);
...
test('管理者新增同事產生認證碼、指派分工；同事用認證碼或連結登入；重新產生後舊碼失效', async () => {
  const admin = client(); await admin.login(ACCOUNTS.admin);
  const item = itemByName('舀杯高手');
  const r = await admin.call('POST', '/api/admin/users', { name: '王老師', role: 'entry', note: '竹東國中', assignments: [{ division_id: junior.id, item_id: item.id }] });
  assert.equal(r.status, 200, JSON.stringify(r.data));
  const code = r.data.user.access_code;
  assert.match(code, /^\d{6}$/, '一般人員為 6 位數字');
  assert.ok(r.data.user.login_link.endsWith(`/staff?code=${code}`));
  const users = (await admin.call('GET', '/api/admin/users')).data.users;
  assert.ok(users.find((u) => u.name === '王老師' && u.access_code === code));
  // 管理者認證碼為 8 位英數
  const ra = await admin.call('POST', '/api/admin/users', { name: '副管理', role: 'admin' });
  assert.match(ra.data.user.access_code, /^[A-Z2-9]{8}$/);

  const c = client();
  assert.equal((await c.login('000000')).status, 401, '錯誤認證碼');
  assert.equal((await c.login(code.slice(0, 3) + ' ' + code.slice(3))).status, 200, '可含空白');
  ...
  // QR 只有管理者能拿
  assert.equal((await fetch(`${BASE}/api/admin/users/${r.data.user.id}/qr.svg`)).status, 401);
  assert.equal((await admin.call('GET', `/api/admin/users/${r.data.user.id}/qr.svg`)).status, 200);
  // 重新產生：舊碼失效、既有登入被登出
  const rg = await admin.call('POST', `/api/admin/users/${r.data.user.id}/regenerate-code`);
  assert.equal(rg.status, 200);
  assert.notEqual(rg.data.user.access_code, code);
  assert.equal((await c.call('GET', '/api/staff/bootstrap')).status, 401);
  assert.equal((await client().login(code)).status, 401);
  assert.equal((await client().login(rg.data.user.access_code)).status, 200);
  // 停用後無法再操作
  const nu = users.find((u) => u.name === '王老師');
  assert.equal((await admin.call('PATCH', `/api/admin/users/${nu.id}`, { is_active: false })).status, 200);
  assert.equal((await client().login(rg.data.user.access_code)).status, 401);
  // 管理者不能停用自己
  const me = users.find((u) => u.access_code === ACCOUNTS.admin);
  assert.equal((await admin.call('PATCH', `/api/admin/users/${me.id}`, { is_active: false })).status, 400);
});
...
test('登入嘗試限制：同一來源連續猜錯會被暫時封鎖', async () => {
  let last;
  for (let i = 0; i < 22; i++) last = await client().login(String(900000 + i));
  assert.equal(last.status, 401);
  assert.match(last.data.error, /嘗試次數過多/);
});
